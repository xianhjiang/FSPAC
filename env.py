...
class Env(object):
    ...
    def reset(self):
        ...
        if self.use_seg:
            ...
            self.moving_seg = items['moving_seg']
            self.moving_seg_tensor = utils.tensor(items['moving_seg']).to(self.device)
            # self.moving_seg = []
            # for i, lab in enumerate(self.labels):
            #     m_seg = (m_segs == lab)*255.
            #     self.moving_seg.append(m_seg)

            self.moved_seg = copy.deepcopy(self.moving_seg_tensor)

        self.prev_score = self.score()
        self.field = None
        self.global_step = 1
        return self.state(), self.prev_score, self.state_seg()
    ...
    def state_seg(self):
        if self.use_seg:
            return torch.cat([self.fixed_seg, self.moving_seg], dim=1).to(self.device)
        else:
            return 0
    ...
    def score(self):
        if self.use_seg:
            ...
            #     utils.numpy_im(self.moved_seg, 1, device=self.device) > 0, [1])

            # -------------------------------------
            # dices = utils.dice(utils.numpy_im(self.fixed_seg, 1, device=self.device) > 0,
            #                    utils.numpy_im(self.moved_seg, 1, device=self.device) > 0, [1])

            dices = utils.dice(utils.numpy_im_round(self.fixed_seg, 1, device=self.device),
                               utils.numpy_im_round(self.moved_seg, 1, device=self.device), [1, 2, 3])
    ...
    def step(self, field):
        ...
        # 主要就是这里的问题
        if self.use_seg:
            self.moved_seg = self.seg_stn(self.moving_seg_tensor, self.field)
        #这里是注释的
        # if self.use_seg:
        #     for i, lab in enumerate(self.labels):
