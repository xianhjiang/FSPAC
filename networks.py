...
class Encoder(nn.Module):
    def __init__(self, nc=3, nf=32, bottle=512, logvar_max=2, logvar_min=-10):
        ...
        self.out = ConvRelu(nf*4, (bottle)*2, (4, 4, 1), 1, 0) # 1, +12
...
class Decoder(nn.Module):
    ...
    def forward(self, x, enc):
        ...
        x = nn.functional.interpolate(x, size=(4, 4, 1), mode='trilinear', align_corners=False)  # 针对心脏 128 128 32 独有
        # x = self.up1(x) # 4
...
class Critic(nn.Module):
    def __init__(self, nc, nf=32, bottle=512):
        ...
        self.conv5 = ConvRelu(nf*4, nf*4, (4, 4, 1), 2, 1) # 8 -> 4
        self.conv7 = Conv(nf*4, bottle, (4, 4, 2), 1, 0) # 1
